...
def fupl(request):
    return render(request,"fileupld.html")
def fupl_post(request):
    description=request.POST['description']
    file=request.FILES['file']
    fobj=fup()
    fobj.description=description
    fobj.file=file
    return('''<script>alert("upload success");window.location="/myapp/fupl/"</script>''')